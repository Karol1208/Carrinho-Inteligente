# Em ui/interface.py (VERSÃO ATUALIZADA COM POP-UPS, RESTR IÇÕES E MODIFICAÇÕES SOLICITADAS)
...
class InterfaceGraficaCarrinho:
    def __init__(self, carrinho: CarrinhoInteligenteAvancado = None, usuario_inicial=None):
        ...
        self.usuario_atual = usuario_inicial  # Pode ser passado do login inicial
        self.modo_admin = False  # Novo: Flag para modo admin
        self.frames_conteudo = {}
        self.botoes_sidebar = {}
        self.labels_status_gavetas = {}  # Para status das gavetas
        self.tree_usuarios = None  # Para aba usuários
        self.tree_pecas = None  # Para aba inventário
        self.tree_pendencias = None
        self.combo_usuario_pendencias = None
        self.tree_historico = None
        self.entry_busca_peca = None
        self.label_usuario_atual = None
        self.label_usuario_header = None
        self.botao_trocar_admin = None  # Botão para trocar para admin
        ...
        if self.usuario_atual:
            ...
            if hasattr(self, 'label_usuario_header'):
                self.label_usuario_header.config(text=f"Bem-vindo, {self.usuario_atual.nome}!", fg=CORES["texto_claro"])


    def abrir_tela_cadastro_rfid(self):
       tela_rfid = TelaCadastroRFID(self.carrinho, root_principal=self.root, callback_atualizar_usuarios=self.atualizar_lista_usuarios)  # CORREÇÃO: Mudado para método existente
       tela_rfid.executar()
    ...
    def configurar_acesso_por_perfil(self, perfil):
        ...
        if hasattr(self, 'botao_trocar_admin'):
            if perfil == "admin":
                self.botao_trocar_admin.pack_forget()  
                self.modo_admin = True
            else:
                self.botao_trocar_admin.pack(pady=5)  
    ...
    def setup_aba_principal(self, parent):
        ...
        frame_rfid = ttk.LabelFrame(frame, text="Acesso por RFID (Usuário Já Validado)")
        ...
        self.label_usuario_atual.grid(row=0, column=0, columnspan=3, pady=10)

        ttk.Button(frame_rfid, text="Ler RFID", command=self.ler_rfid).grid(row=0, column=1, padx=10, pady=10, ipady=5)

        self.botao_trocar_admin = ttk.Button(frame_rfid, text="Trocar para Admin via RFID", command=self.ler_rfid_admin)
    ...
    def ler_rfid(self):
        try:
            ...
            messagebox.showinfo("RFID", "Hardware RFID não configurado. Use simulação.")

    def ler_rfid_admin(self):
        ...
        try:
            ...
            if codigo:
                ...
                if usuario and usuario.perfil == "admin":
                    ...
                    self.modo_admin = True
                    self.atualizar_info_usuario_atual()
                    self.configurar_acesso_por_perfil("admin")
                    messagebox.showinfo("Sucesso", f"Modo Admin ativado: {usuario.nome}")
                else:
                    messagebox.showwarning("Aviso", "Apenas admins podem ativar este modo.")
            else:
                messagebox.showinfo("Info", "Nenhum RFID detectado.")
        except AttributeError:
            messagebox.showerror("Erro", "Hardware RFID não configurado.")
    ...
    def processar_cartao(self, codigo_cartao):
        ...
        if usuario:
            ...
            if usuario.perfil == "admin":
                messagebox.showinfo("Admin Logado", "Acesso admin ativado!")
        else:
            self.label_usuario_atual.config(text="RFID não autorizado!", foreground=CORES["alerta"])
            self.usuario_atual = None
            self.configurar_acesso_por_perfil(None)

    def atualizar_info_usuario_atual(self):
        if self.usuario_atual:
            self.label_usuario_atual.config(text=f"Usuário: {self.usuario_atual.nome} ({self.usuario_atual.perfil})")
        else:
            self.label_usuario_atual.config(text="Nenhum usuário logado")
    ...
    def mostrar_pop_up_retirada(self, gaveta_id):
        ...
        def fechar_gaveta():
            if self.carrinho.fechar_gaveta(gaveta_id, self.usuario_atual.id if self.usuario_atual else None):
                messagebox.showinfo("Sucesso", f"Gaveta {gaveta_id} fechada com sucesso.", parent=popup)
                popup.destroy()
                self.atualizar_status_gavetas()
            else:
                messagebox.showerror("Erro", "Falha ao fechar gaveta. Verifique permissões.", parent=popup)
        ...
        ttk.Button(frame_botoes, text="Fechar Gaveta", command=fechar_gaveta).pack(side="left", padx=10)
    ...
    def setup_aba_usuarios(self, parent):
        ...
        ttk.Button(
            ...
            command=self.abrir_tela_cadastro_rfid
        ).pack(pady=10)
        ...
        scrollbar_usuarios = ttk.Scrollbar(
            frame_lista,
            orient='vertical',
    ...
    def mostrar_popup_cadastro_usuario(self, id_cartao_preenchido=None):
        popup = tk.Toplevel(self.root)
        ...
        # Pré-preencher ID do Cartão se fornecido pela tela de cadastro RFID
        if id_cartao_preenchido:
            entries["ID do Cartão:"].insert(0, id_cartao_preenchido)
        ...
        def salvar_novo_usuario():
            ...
            usuario = UsuarioCartao(id=id_cartao, nome=nome, cargo=cargo, perfil=perfil, data_cadastro=datetime.datetime.now().isoformat(), ativo=True)
            try:
    ...
    def atualizar_combo_usuarios_pendencias(self):
        ...
        if hasattr(self, 'combo_usuario_pendencias'):
    ...
    def atualizar_pendencias_usuario(self, event=None):
        if not hasattr(self, 'combo_usuario_pendencias'):
            return
        ...
        if hasattr(self, 'tree_pendencias'):
